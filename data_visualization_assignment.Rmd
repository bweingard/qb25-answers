...
ggplot(data = dt, aes(x = Liver)) + geom_histogram( fill = "yellow", breaks = seq(0,100,2)) + labs(title = "Liver Transcriptome", x = "Expression Level", y = "Frequency")

# This shows a histogram that is skewed to the right. The median value is 0 or close to zero. There are thousands of genes skewed to the right, with the frequency of genes being expressed decreases with increasing expression.


ggplot(data = dt, aes(x = `Heart - Left Ventricle`)) + geom_histogram(fill = "red", breaks = seq(0,100,2)) + labs(title = "Left Ventricle Transcriptome", x = "Expression Level", y = "Frequency")

#The heart transcriptome is the same. If we filter by expressed genes with a TPM>1 we will see a better representation.
...
```{r q2}
ggplot(data = dt, aes(x = Liver)) + geom_density(fill = "orange") + labs(title = "Liver Transcriptome", x = "Expression Level", y = "Frequency")

expression_values <- (dt$Liver +1)
ggplot(data = dt, aes(x = expression_values)) + geom_density(fill = "purple") + labs(title = "Liver Transcriptome", x = "Expression Level", y = "Frequency") + scale_x_log10()
# answer here
```
...
```{r q3}
expression_values <- (dt$Liver +1)
expression_values2 <- (dt$`Heart - Left Ventricle`+1)

ggplot(data = dt, aes(x = expression_values, y = expression_values2)) + geom_point(size=1, alpha = 0.2, color = "navy") + labs(title = "Liver vs Heart Left Ventricle (LV) Transcriptome", x = "Liver Expression", y = "Heart LV Expression") + scale_x_log10()+ scale_y_log10() + geom_abline(slope = 1, color = "red", lty = 2) + theme_minimal()
# answer here
```
...
```{r q4}
dt_scatter <- dt %>% mutate(is_mitochondrial = str_starts(Description, 'MT-'))

ggplot(data = dt_scatter, aes(x = expression_values, y = expression_values2, color = is_mitochondrial)) +
  geom_point(size=1, alpha = 0.2) + 
  labs(title = "Liver vs Heart Left Ventricle (LV) Transcriptome", x = "Liver Expression", y = "Heart LV Expression") + 
  scale_x_log10()+ 
  scale_y_log10() + 
  geom_abline(slope = 1, color = "red", lty = 2) +
  theme_minimal()+
  scale_color_brewer(palette = "Pastel1", name = "Is it Mitochondrial?")
# answer here
```
...
ggplot(data = dt_long, aes(x = TPM, fill = Tissue)) +
  geom_histogram(breaks = seq(0,100,2)) +
  labs(title = "Transcriptome", x = "Expression Level", y = "Tissue")+
  facet_grid(.~Tissue)
